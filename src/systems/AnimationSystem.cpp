...
void AnimationSystem::tick(World* world) {
   ...
   world->find<AnimationComponent, TextureComponent, SpritesheetComponent>([&](Entity* entity) {
   ...
   world
       ->find<AnimationComponent, PausedAnimationComponent, TextureComponent, SpritesheetComponent>(
           [&](Entity* entity) {
              if (!Camera::Get().inCameraRange(entity->getComponent<PositionComponent>()) &&
                  !entity->hasComponent<IconComponent>()) {
                 return;
